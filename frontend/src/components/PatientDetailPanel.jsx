  ...
  return (
    <div className="panel-card detail-card">
      <div className="patient-heading">
        {patient.foto_url ? <img className="patient-photo" src={`${(import.meta.env.VITE_API_URL || 'http://localhost:4000/api').replace('/api', '')}${patient.foto_url}`} alt={`Paciente ${patient.nombre}`} /> : <div className="patient-photo patient-photo-empty">{patient.nombre.slice(0, 1)}</div>}
